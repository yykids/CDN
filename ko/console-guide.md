...
> [주의]
> 도메인 별칭을 등록한 경우 기본으로 제공된 CDN 서비스 도메인 `*.cdn.toastcloud.com`은 퍼지가 되지 않습니다. 
> 등록한 도메인 별칭 주소에 대해서만 퍼지가 제공되므로 주의해주세요. 
...
> 
> 예를 들어 `custom1.domain-alias.com` 과 `custom2.domain-alias.com` 이 도메인 별칭으로 설정된 CDN 서비스가 있다고 가정합니다.
> 위 CDN 서비스를 **특정 파일** 타입의 퍼지 경로 `/images/photo.png` 로 퍼지를 요청 한 경우 아래의 2개의 퍼지 경로로 퍼지가 수행됩니다. 
> - custom1.domain-alias.com/images/photo.png
> - custom2.domain-alias.com/images/photo.png 
> 퍼지 경로 수 = (등록된 도메인 별칭 수 X 요청한 퍼지 경로 수) 만큼 퍼지 경로가 추가되어 퍼지가 진행되므로 유의해주세요. 
> 만일 퍼지 경로 수가 퍼지 사용량 제한의 요청 당 퍼지 경로 수를 초과한 경우에는 전체 경로에 대해 요청 당 가능한 퍼지 경로 수만큼씩 나누어 퍼지 작업이 진행됩니다. 
> 나누어 진 퍼지 작업 수 만큼 퍼지 사용량 제한의 사용량도 증가하므로 유의해주세요. 
