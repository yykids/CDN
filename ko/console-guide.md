...
- Referrers 접근 관리  
  특정 referrer의 경우 사용자 콘텐츠에 접근을 제한할 수 있습니다.  
...
- CDN 서비스 생성 및 수정, 재시작 후 "서비스 중" 상태로 바뀌었을 때 특정 URL을 호출할 필요가 있을 때 설정할 수 있습니다. 지원되는 HTTP 메서드는 GET, POST, PUT 메서드입니다.


8.[확인] 버튼을 클릭하면 CDN 서비스 생성 요청이 완료됩니다.  
생성 요청 이후 서비스 배포가 완료되기까지 수 분 정도 (최대 한시간) 소요됩니다. 배포가 완료된 후 서비스 이용이 가능합니다.
...
3.누적/순간 트래픽 타입 설정 후 제한 할 트래픽 양을 지정합니다. 단위는 Byte입니다.  
...
네트워크 전송량, Status 코드별 통계 및 Top 콘텐츠에 대한 통계를 확인할 수 있습니다. 
