...
- **Domain Alias**
  ...
  하지만 개인 또는 회사가 소유한 도메인을 이용해 CDN 서비스를 제공해야 할 때는, **Domain Alias** 설정을 이용할 수 있습니다.
  **Domain Alias**를 이용하면 TOAST에서 제공하는 도메인 외에 개인 혹은 회사가 소유한 도메인으로도 CDN 서비스를 사용할 수 있습니다.
  **Domain Alias**에 소유한 도메인 주소를 입력하고, 해당 도메인의 네임 서버 설정을 변경해 주세요.
...
- **리퍼러(referrer) 헤더 접근 관리**
  리퍼러 접근 관리를 설정합니다.
  특정 웹 페이지에서 사용자 콘텐츠를 링크하여 리소스를 요청하는 경우 리퍼러 헤더에 링크한 URL의 경로가 추가되어 요청되므로 어떤 경로에서 유입되었는지 알 수 있습니다.
  리퍼러 접근 관리는 요청 헤더의 리퍼러 헤더값에 따라 사용자 콘텐츠에 대한 접근을 제한할 수 있는 기능입니다. 리퍼러는 정규 표현식 형태로 입력할 수 있습니다.
	* Blacklist(블랙리스트) 타입:
...
> 특수 문자로의 의미가 아닌 문자 그대로 해석이 필요하다면 이스케이프 문자 백슬래시(`\`)를 특수 문자 앞에 추가해야 합니다.
...
1. **HTTP Method**와 **Callback URL**을 입력합니다.
2. Request URI의 Query Parameter로 CDN 서비스 변경 작업에 대한 결과를 받으려면 **Callback URL**에 Path 변수를 포함해 입력해 주세요.
...
* **확인** 버튼을 클릭하면 CDN 서비스 생성 요청이 완료됩니다.
...
* 변경할 수 있는 항목은 설명, 원본 서버 정보, Domain Alias, Callback입니다.
...
1. 변경을 원하는 서비스를 CDN 서비스 목록에서 선택합니다.
2. **Cache 설정** 탭의 **수정** 버튼을 클릭합니다.

![cdn_05_201812](https://static.toastoven.net/prod_cdn/cdn_05_201812.png)

3. **캐시 만료 설정**에서 만료 설정 방식을 선택합니다.

> [주의]
> 만일 원본 서버 설정에 캐시 만료 시간이 지정되어 있다면 입력한 **Cache 만료 시간(초)**값은 무시됩니다. CDN 서비스를 이용해 만료 시간을 지정하고 싶은 경우 원본 서버의 캐시 만료 설정을 제거합니다.

* 원본 설정 사용
    원본 서버의 캐시 만료 설정을 이용하도록 설정합니다.
    * 사용자 설정 사용
    원하는 캐시 만료 시간을 설정할 수 있습니다.
* 리퍼러를 이용해 접근을 관리할 수 있습니다.
    특정 리퍼러만 사용자 콘텐츠의 접근을 허용하거나 거부하도록 설정할 수 있습니다. 정규 표현식 형태로 입력하고 여러 개의 리퍼러를 제어할 경우 입력 창에 라인을 추가하여 입력합니다.
* 캐시 설정 후 특정 URL을 호출해야 할 때는 Callback URL을 설정할 수 있습니다.
4. **확인** 버튼을 클릭해 캐시 설정을 변경합니다.
...
3. 캐시 재배포 타입을 선택합니다. 3가지 타입 중에 선택할 수 있습니다.
  * Item: 정확한 파일 이름과 경로를 설정해 원하는 파일만 재배포할 수 있습니다.
      * 예) /path/to/file1.jpg
  * Wildcard: 파일 이름과 경로 이름에 와일드카드 문자를 이용할 수 있습니다.
      * \*: 임의의 문자열
      * ?: 1개의 문자
      * \: Escape 문자
          * 예) /images/games/\*.jpg
...
> 
> 
> * ITEM 타입: 시간당 60회 제한, 한 번에 요청할 수 있는 최대 Path 수는 1000개
> * Wildcard 타입: 시간당 60회 제한, 한 번에 요청할 수 있는 최대 Path 수는 10개
> * ALL 타입: 시간당 5회 제한
